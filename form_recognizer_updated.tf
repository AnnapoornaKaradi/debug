...
resource "null_resource" "form_recognizer_connection_key_rotation" {
	...
	provisioner "local-exec" {
		command = <<EOT
			echo "Waiting for resource to reach terminal provisioning state...";
			for i in $(seq 1 30); do
				STATE=$(/usr/bin/az cognitiveservices account show \
					--name $NAME --resource-group $RG_NAME \
			...
			if [ "$STATE" != "Succeeded" ]; then
				echo "ERROR: Resource did not reach Succeeded state after 300s. Aborting key rotation.";
				exit 1;
			fi;
			/usr/bin/az cognitiveservices account keys regenerate --key-name Key1 --name $NAME --resource-group $RG_NAME > /dev/null || exit 1;
			/usr/bin/az cognitiveservices account keys regenerate --key-name Key2 --name $NAME --resource-group $RG_NAME > /dev/null || exit 1;
			echo "Key rotation completed successfully for $NAME";
		EOT
	...
	depends_on = [ azurerm_cognitive_account.form ]
}
