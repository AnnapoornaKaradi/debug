...
locals {
	...
	aks_managed_prometheus_enabled = { for k, v in local.aks_instances :
		k => (
			lookup(lookup(v.enable_managed_prometheus, "env_override",{}), local.my_env_short, null) != null ? v.enable_managed_prometheus.env_override[local.my_env_short] : v.enable_managed_prometheus.default
		)
	}
}

#----- Wire each AKS cluster to the shared Azure Monitor Workspace for Managed Prometheus
#      This creates the MSProm DCR, DCE, and DCR association automatically in the env subscription
resource "null_resource" "aks_enable_managed_prometheus" {
	for_each = { for k, v in local.aks_instances : k => v if lookup(lookup(v.enable_managed_prometheus, "env_override",{}), local.my_env_short, null) != null ? v.enable_managed_prometheus.env_override[local.my_env_short] : v.enable_managed_prometheus.default }

	triggers = {
		cluster_id    = azurerm_kubernetes_cluster.env[each.key].id
		workspace_id  = data.terraform_remote_state.common.outputs.azure_monitor_workspace.id
	}

	provisioner "local-exec" {
		command = <<EOT
			/usr/bin/az aks update \
				--subscription ${data.azurerm_subscription.current.subscription_id} \
				--resource-group ${azurerm_kubernetes_cluster.env[each.key].resource_group_name} \
				--name ${azurerm_kubernetes_cluster.env[each.key].name} \
				--enable-azure-monitor-metrics \
				--azure-monitor-workspace-resource-id ${data.terraform_remote_state.common.outputs.azure_monitor_workspace.id} \
	...
	depends_on = [
		...
		null_resource.register_microsoft_monitor,
	]
}
...
#----- Register the Microsoft.Monitor provider (required for Azure Monitor Workspace / Managed Prometheus)
resource "null_resource" "register_microsoft_monitor" {
	provisioner "local-exec" {
		command = <<EOT
			/usr/bin/az provider register -n Microsoft.Monitor || exit 1;

			elapsed_time=0;

			while [ "`/usr/bin/az provider show -n Microsoft.Monitor | jq -r .registrationState`" != "Registered" ]; do
				sleep 5;

				elapsed_time=$((elapsed_time + 5));

				if [ $elapsed_time -ge 600 ]; then
					echo "Timeout reached waiting for Microsoft.Monitor provider registration.";
					exit 1;
				fi
			done
		EOT
...
resource "azurerm_kubernetes_cluster" "env" {
	...
	default_node_pool {
		...
		upgrade_settings {
			max_surge = each.value.nodepools["default"].max_surge
		}
			...
			[ for nodepool_key, nodepool_value in instance_value.nodepools :
				{
					...
					max_surge = nodepool_value.max_surge
					orchestrator_version = lookup(lookup(lookup(nodepool_value, "orchestrator_version",{}),"env_override", {}), local.my_env_short, null) != null ? nodepool_value.orchestrator_version.env_override[local.my_env_short] : local.kubernetes_version[instance_key]
				}
			]
		]
